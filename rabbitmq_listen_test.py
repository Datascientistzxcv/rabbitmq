    ...
    def callback(ch, method, properties, body):
        for queue_data in json.loads(body):
            ...
            if len(data)!=0:
                ...
                company_name_ratio,position_ratio,description_ratio,duration_ratio=experience_ratios(data,queue_data)
                skills_ratio=skills_ratios(data,queue_data)
                education_ratio=education_ratios(data,queue_data)
                social_url_ratio=social_url_ratios(data,queue_data)

                if len(company_name_ratio)>=0 and name_ratio>=70:
                    with Dbconnect() as client:
                        client.update_freelanceMap(data[0]["_id"],queue_data["Profile Url"])
                elif (len(position_ratio)>=2 or len(description_ratio)>=2 or len(duration_ratio)>=2) and (len(company_name_ratio)>=0 or name_ratio>=70):
                    with Dbconnect() as client:
                        client.update_freelanceMap(data[0]["_id"],queue_data["Profile Url"])
                elif len(education_ratio)>=1 and (len(company_name_ratio)>=1 or  name_ratio>=70):
                    with Dbconnect() as client:
                        client.update_freelanceMap(data[0]["_id"],queue_data["Profile Url"])
                elif len(social_url_ratio)>=1 and (len(company_name_ratio)>=70 or name_ratio>=70):
                    with Dbconnect() as client:
                        client.update_freelanceMap(data[0]["_id"],queue_data["Profile Url"])
                else:
                    pass
